...
//-----------------------------------------------------------------------------------------
public class Item_Spawn : MonoBehaviour  // 이것은 주인공이 발사하는 직선형 스킬을 구현한 스크립트 이다.
{
    ///============================================================================================


    public GameObject[] Item;    // 복제할 미사일 오브젝트
    ///=============================================================================================
    public Transform[] Item_Location;   // 미사일이 발사될 위치
    ...
    private Item_MemoryPool MPool;           // 메모리 풀
    ...
    private void Start()
    {
        ...
        // 메모리 풀을 초기화합니다.
        MPool = new Item_MemoryPool();
        // PlayerMissile을 MissileMaxPool만큼 생성합니다.
        MPool.Create(Item, MissileMaxPool);
        ...
        if (FireState)
        {
            ...
            for (int i = 0; i < MissileMaxPool; i++)
            {
                ...
                if (MissileArray[i] == null)
                {
                    ...
                    MissileArray[i].transform.position = Item_Location[Random.Range(0, Item_Location.Length)].transform.position;
